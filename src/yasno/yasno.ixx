...
namespace ysn
{
	std::optional<wil::com_ptr<ID3D12Resource>> CreateCubemapTexture()
	{
		...
		{
			std::cerr << "Can't create cubemap resource\n";
			return std::nullopt;
		}
	...
	bool Yasno::CreateGpuCameraBuffer()
	{
		...
		if (hr != S_OK)
		{
			std::cerr << "Can't create camera GPU buffer\n";
			return false;
		}
	...
	bool Yasno::CreateGpuSceneParametersBuffer()
	{
		...
		if (hr != S_OK)
		{
			std::cerr << "Can't create scene parameters GPU buffer\n";
			return false;
		}
	...
	bool Yasno::LoadContent()
	{
		...
		// Finish index buffer
		{
			// Index buffer
			{
				...
				{
					std::cerr << "Can't create indices buffer\n";
					return false;
				}
			...
			// Vertex Buffer
			{
				...
				{
					std::cerr << "Can't create vertices buffer\n";
					return false;
				}
			...
			// Material buffer
			{
				...
				{
					std::cerr << "Can't create materials buffer\n";
					return false;
				}
			...
			// Instance buffer
			{
				...
				{
					std::cerr << "Can't create RenderInstance buffer\n";
					return false;
				}
		...
		{
			std::cerr << "Can't initialize tonemap pass\n";
			return false;
		}

		//if (!m_convert_to_cubemap_pass.Initialize())
		//{
		//	std::cerr << "Can't initialize convert to cubemap pass\n";
		//	return false;
		//}

		if (!m_skybox_pass.Initialize())
		{
			std::cerr << "Can't initialize skybox pass\n";
			return false;
		}
		...
		{
			std::cerr << "Yasno app can't create GPU camera buffer\n";
			return false;
		}

		if (!CreateGpuSceneParametersBuffer())
		{
			std::cerr << "Yasno app can't create GPU scene parameters buffer\n";
			return false;
		}
		...
		{
			std::cerr << "Can't initialize forward pass\n";
			return false;
		}
		...
		{
			std::cerr << "Can't initialize raytracing pass\n";
			return false;
		}
