...
namespace ysn
{
		...
		ID3D12Resource* previous_result)
	{
		...
		descriptor_buffer.Resource()->Map(0, nullptr, reinterpret_cast<void**>(&instanceDescs));
		...
		for (uint32_t i = 0; i < instanceCount; i++)
		{
			...
			instanceDescs[i].AccelerationStructure = m_instances[i].blas.GPUVirtualAddress();
			instanceDescs[i].InstanceMask = 0xFF;
		}

		descriptor_buffer.Resource()->Unmap(0, nullptr);

		D3D12_GPU_VIRTUAL_ADDRESS pSourceAS = update_only ? previous_result->GetGPUVirtualAddress() : 0;
		...
		buildDesc.Inputs.InstanceDescs = descriptor_buffer.GPUVirtualAddress();
		buildDesc.Inputs.NumDescs = instanceCount;
		buildDesc.DestAccelerationStructureData = result_buffer.GPUVirtualAddress();
		buildDesc.ScratchAccelerationStructureData = scratch_buffer.GPUVirtualAddress();
		...
		uavBarrier.UAV.pResource = result_buffer.Resource();
