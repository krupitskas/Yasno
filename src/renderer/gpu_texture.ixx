...
namespace ysn
{
	...
	std::optional<GpuTexture> LoadTextureFromFile(const LoadTextureParameters& parameters)
	{
		...
		if (ldr_data != nullptr || hdr_data != nullptr)
		{
			...
			if (result != S_OK)
			{
				std::cerr << "Can't create GPU texture " << parameters.filename << "\n";
				return std::nullopt;
			}
			...
			if (result != S_OK)
			{
				std::cerr << "Can't create GPU upload heap texture " << parameters.filename << "\n";
				return std::nullopt;
			}
		...
		{
			std::cerr << "Can't load " << parameters.filename << " texture\n";
			return std::nullopt;
		}
