...
std::optional<wil::com_ptr<IDXGIAdapter4>> CreateDXGIAdapter()
{
	...
	{
		std::cerr << "Can't create DXGIFactory2 with flags " << create_factory_flags << ", error is: " << ysn::ConvertHrToString(err) << " aborting!\n";
		return std::nullopt;
	}
...
namespace ysn
{
	bool DxRenderer::Initialize()
	{
		...
		{
			std::cerr << "Can't create shader manager\n";
			return false;
		}
		...
		//{
		//	std::cerr << "Can't initialize generate mips system\n";
		//	return false;
		//}
	...
	bool DxRenderer::CreateRootSignature(D3D12_ROOT_SIGNATURE_DESC* pRootSignatureDesc, ID3D12RootSignature** ppRootSignature) const
	{
		...
		if (FAILED(hr))
		{
			std::cerr << "Can't create root signature: " << ConvertHrToString(hr) << "\n";
			return false;
		}
	...
	bool DxRenderer::CheckRaytracingSupport()
	{
		...
		{
			std::cerr << "Ray tracing is not supported";
			return false;
		}

		if (options5.RaytracingTier < D3D12_RAYTRACING_TIER_1_0)
		{
			std::cerr << "Ray tracing tier less then 1.0";
			return false;
		}

		std::cout << "RTX is supported\n";
