...
namespace ysn
{
	static bool CreateParameters(wil::com_ptr<ID3D12Resource>& parameters_buffer)
	{
		...
		auto hr = device->CreateCommittedResource(&heapProperties, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&parameters_buffer));

		if (hr != S_OK) 
		{
			std::cerr << "Can't create tonemap parameters buffer\n";
			return false;
		}
	...
	bool TonemapPostprocess::Initialize()
	{
		...
		{
			std::cerr << "Can't load tonemap shader\n";
			return false;
		}
		...
		if (auto result = Application::Get().GetDevice()->CreatePipelineState(&pipeline_state_stream_desc, IID_PPV_ARGS(&pipeline_state)); result != S_OK)
		{
			std::cerr << "Can't create tonemap pipeline state\n";
			return false;
		}

		if(!CreateParameters(parameters_buffer))
		{
			std::cerr << "Can't create tonemap parameters\n";
			return false;
		}
	...
	void TonemapPostprocess::Render(TonemapPostprocessParameters* parameters)
	{
		...
		CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(parameters->scene_color_buffer.get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
