...
namespace ysn
{
	bool ConvertToCubemap::Initialize()
	{
		...
		root_parameters[ShaderInputParameters::ViewProjection].InitAsConstantBufferView(0);
		root_parameters[ShaderInputParameters::InputTexture].InitAsDescriptorTable(1, &cubemap_srv);

		CD3DX12_STATIC_SAMPLER_DESC static_sampler(0, D3D12_FILTER_MIN_MAG_MIP_POINT, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, D3D12_TEXTURE_ADDRESS_MODE_CLAMP);
		...
		vs_parameters.shader_path = ysn::GetVirtualFilesystemPath(L"Shaders/ConvertEquirectangularMap_VS.hlsl");
		...
		ps_parameters.shader_path = ysn::GetVirtualFilesystemPath(L"Shaders/ConvertEquirectangularMap_PS.hlsl");
