...
namespace ysn
{
									...
									uint32_t primitives_count)
	{
		...
		{
			std::cerr << "Can't create raytracing pass pipeline\n";
			return false;
		}
		...
		if (!CreateShaderBindingTable(renderer,
			...
			primitives_count))
		{
			std::cerr << "Can't create raytracing pass shader binding table\n";
			return false;
		}
	...
	bool RaytracingPass::CreateRaytracingPipeline(std::shared_ptr<ysn::DxRenderer> renderer)
	{
		...
		{
			std::cerr << "Can't compile ray gen shader\n";
			return false;
		}
		...
		{
			std::cerr << "Can't compile miss shader\n";
			return false;
		}
		...
		{
			std::cerr << "Can't compile hit shader\n";
			return false;
		}
		...
		{
			std::cerr << "Can't QueryInterface rt_state_object_props from CreateRaytracingPipeline\n";
			return false;
		}
												  ...
												  uint32_t primitives_count)
	{
		...
		m_sbt_storage = CreateBuffer(renderer->GetDevice().get(),
									 sbtSize,
		...
		if (!m_sbt_storage)
		{
			std::cerr << "Could not allocate the shader binding table\n";
			return false;
		}
								 ...
								 wil::com_ptr<ID3D12Resource> camera_buffer)
	{
		...
		CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(scene_color.get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
