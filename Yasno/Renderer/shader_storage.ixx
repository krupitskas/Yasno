...
namespace ysn
{
	...
	bool ShaderStorage::Initialize()
	{
		...
		// Some common include files
		{
			...
			if (hr != S_OK)
			{
				std::cerr << "Can't create include handler\n";
				return false;
			}
	...
	std::optional<wil::com_ptr<IDxcBlob>> ShaderStorage::CompileShader(const ShaderCompileParameters& parameters)
	{
		...
		std::cout << "Compiling shader: " << fullpath.string().c_str() << "\n";
		...
		switch (parameters.shader_type)
		{
			case ShaderType::None:
				std::cerr << "... has None as shader type\n";
				return std::nullopt;
		...
		{
			std::cerr << "Cannot find shader file :" << WStringToString(parameters.shader_path) << "\n";
			return std::nullopt;
		}
		...
		{
			std::cerr << "Can't create shader blob\n";
			return std::nullopt;
		}
		...
		{
			std::cerr << "Can't get status after shader compilation\n";
			return std::nullopt;
		}
		...
		if (FAILED(result_code))
		{
			...
			if (FAILED(result))
			{
				std::cerr << "Failed to get shader compiler error\n";
				return std::nullopt;
			}
		...
		{
			std::cerr << "Can't get shader blob DXC_OUT_OBJECT\n";
			return std::nullopt;
		}
		...
		{
			std::cerr << "Can't get shader blob DXC_OUT_PDB\n";
			return std::nullopt;
		}
		...
		if(shader_debug_data && shader_debug_data_path)
		{
			...
			if (CreateDirectoryIfNotExists(pdb_path))
			{
				...
				{
					std::cerr << "Can't store pdb data\n";
				}
			}
			else
			{
				std::cerr << "Can't create pdb storage directory \n";
			}
		}
		...
		{
			std::cerr << "Can't get shader blob DXC_OUT_REFLECTION\n";
			return std::nullopt;
		}
		...
		{
			std::cerr << "Can't get shader blob DXC_OUT_SHADER_HASH\n";
			return std::nullopt;
		}
		...
		//	else
		//	{
		//		std::cerr << "Can't get shader modification time: " << WStringToString(parameters.shader_path) << "\n";
		//	}
		//#endif
	...
	//				{
	//					std::cout << "Shader " << WStringToString(shader_path) << " was modified, recompiling PSO\n";
	//
	...
	//			else
	//			{
	//				std::cerr << "Can't get shader modification time: " << WStringToString(shader_path) << "\n";
	//			}
	//		}
	//	}
	//
