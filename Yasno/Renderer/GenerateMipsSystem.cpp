...
namespace ysn
{
	bool GenerateMipsSystem::Initialize(const DxRenderer& renderer)
	{
		...
		{
			LogFatal << "Generate mip pass can't create root signature\n";
			return false;
		}
		...
		{
			LogFatal << "Can't load generate mips shader\n";
			return false;
		}
		...
		if (auto result = renderer.GetDevice()->CreatePipelineState(&pipeline_state_stream_desc, IID_PPV_ARGS(&m_pipeline_state)); result != S_OK)
		{
			LogFatal << "Can't create generate mips pipeline state\n";
			return false;
		}
	...
	bool GenerateMipsSystem::GenerateMips(std::shared_ptr<DxRenderer> renderer, wil::com_ptr<ID3D12GraphicsCommandList> command_list, const GpuTexture& gpu_texture)
	{
		...
		{
			CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(resource.get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
		...
		for (uint32_t src_mip = 0; src_mip < resource_desc.MipLevels - 1u; )
		{
			...
			uav_orig_desc.Format				= DXGI_FORMAT_R8G8B8A8_UNORM; // TODO: If srgb -> rollback to DXGI_FORMAT_R8G8B8A8_UNORM, check DXGI_FORMAT PixelBuffer::GetUAVFormat( DXGI_FORMAT defaultFormat )
		...
		{
			CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(resource.get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			command_list->ResourceBarrier(1, &barrier);
		}
