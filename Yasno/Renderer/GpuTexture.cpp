...
namespace ysn
{
	...
	std::optional<GpuTexture> LoadTextureFromFile(const LoadTextureParameters& parameters)
	{
		...
		if (ldr_data != nullptr || hdr_data != nullptr)
		{
			...
			if (result != S_OK)
			{
				LogFatal << "Can't create GPU texture " << parameters.filename << "\n";
				return std::nullopt;
			}
			...
			if (result != S_OK)
			{
				LogFatal << "Can't create GPU upload heap texture " << parameters.filename << "\n";
				return std::nullopt;
			}
			...
			UpdateSubresources(parameters.command_list.get(), result_texture.gpu_resource.get(), texture_upload_heap.get(), 0, 0, 1, &texture_data);
			...
			//const CD3DX12_RESOURCE_BARRIER barrier_desc = CD3DX12_RESOURCE_BARRIER::Transition(result_texture.gpu_resource.get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
			//parameters.command_list->ResourceBarrier(1, &barrier_desc);
		}
		else
		{
			LogFatal << "Can't load " << parameters.filename << " texture\n";
			return std::nullopt;
		}
