...
namespace ysn
{
	std::optional<GpuBuffer> CreateGpuBuffer(const GpuBufferCreateInfo& info, const std::string& debug_name)
	{
		...
		const HRESULT hr = d3d_device->CreateCommittedResource(&heap_desc, D3D12_HEAP_FLAG_NONE, &resource_desc, info.state, nullptr, IID_PPV_ARGS(&result_buffer.resource));

		if (hr != S_OK)
		{
			std::cerr << "Can't create GPU buffer " << debug_name;
			return std::nullopt;
		}
		else
		{
	...
	bool UploadToGpuBuffer(wil::com_ptr<ID3D12GraphicsCommandList4> cmd_list, const GpuBuffer& target_buffer, const void* upload_data, const std::vector<D3D12_SUBRESOURCE_DATA>& subresource_data, D3D12_RESOURCE_STATES target_state)
	{
		...
		{
			std::cerr << "Can't create gpu upload buffer\n";
			return false;
		}
		...
		UINT64 uploaded_bytes = UpdateSubresources(cmd_list.get(), target_buffer.resource.get(), upload_buffer.resource.get(), 0, 0, subresource_count, buffer_data_desc_ptr);
		HRESULT hr = (required_size == uploaded_bytes ? S_OK : E_FAIL);

		if (hr != S_OK)
		{
			std::cerr << "Failed to upload data via upload heap\n";
			return false;
		}
