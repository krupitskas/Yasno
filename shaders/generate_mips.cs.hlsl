...
void main(ComputeShaderInput IN)
{
	...
	OutMip1[IN.DispatchThreadID.xy] = PackColor(Src1); // float4(1, 0, 0, 0);//
	...
	if ((IN.GroupIndex & 0x9) == 0)
	{
		...
		OutMip2[IN.DispatchThreadID.xy / 2] = PackColor(Src1); // float4(0,1,0,0);//
	...
	if ((IN.GroupIndex & 0x1B) == 0)
	{
		...
		OutMip3[IN.DispatchThreadID.xy / 4] = PackColor(Src1); // float4(0,0,1,0);//
	...
	if (IN.GroupIndex == 0)
	{
		...
		OutMip4[IN.DispatchThreadID.xy / 8] = PackColor(Src1); // float4(1,0,1,0);//
	}
}
